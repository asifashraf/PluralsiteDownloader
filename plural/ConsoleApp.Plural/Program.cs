        ...
        static void timer_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            if (items.Count > 0)
            {
                ...
                using (var webClient = new WebClient())
                {
                    try
                    {
                        var json = webClient.DownloadString(string.Format(settings.COURSE_DATA_URL, name));
                        course = JsonConvert.DeserializeObject<Course>(json);

                        var url = String.Format("{0}/Home/Index?course={1}", settings.plural, name);
                        ...
                        if (items.Count > 0)
                        {
                            ...
                            var nextInterval = Convert.ToDouble(Math.Floor(
                                Convert.ToDecimal(span.TotalMilliseconds) / settings.division_factor));
